...
export async function importAllModules(importMeta: ImportMeta, sourceDir: string): Promise<void> {
  const exclude = [
    "**/*.test.*",
    "**/*.spec.*",
    "**/__tests__/**",
  ...
  const absoluteSourceDir = resolve(dirname(fileURLToPath(importMeta.url)), sourceDir);
  const pattern =
