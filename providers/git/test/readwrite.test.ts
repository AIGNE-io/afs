...
test("should delete file", async () => {
  ...
  // Verify it exists
  const result = await afsGit.read("/main/delete-me.txt");
  expect(result.data?.content).toBe("To be deleted\n");

  // Delete it
  await afsGit.delete("/main/delete-me.txt");

  // Verify it's gone - should throw AFSNotFoundError
  await expect(afsGit.read("/main/delete-me.txt")).rejects.toBeInstanceOf(AFSNotFoundError);
});
...
test("should rename file", async () => {
  ...
  // Old path should not exist - should throw AFSNotFoundError
  await expect(afsGit.read("/main/old-name.txt")).rejects.toBeInstanceOf(AFSNotFoundError);
