...
test("should list all branches at root", async () => {
  ...
  // All branches should have childrenCount defined
...
test("should throw AFSNotFoundError for non-existent file", async () => {
  await expect(afsGit.read("/main/non-existent.txt")).rejects.toBeInstanceOf(AFSNotFoundError);
});
...
test("should list only branches at root with maxDepth: 1", async () => {
  const result = await afsGit.list("/", { maxDepth: 1 });

  const paths = result.data.map((e) => e.path).sort();

  // Should only have branches, no files inside branches
  expect(paths).toContain("/main");
  expect(paths).toContain("/develop");
  expect(paths).toContain("/feature-test");

  // Should NOT include files from branches
  expect(paths.some((p) => p.includes("/main/"))).toBe(false);
  expect(paths.some((p) => p.includes("/develop/"))).toBe(false);
});

test("should list branches and their contents at root with maxDepth: 2", async () => {
  const result = await afsGit.list("/", { maxDepth: 2 });
  ...
  // Should also have root-level files from branches
  expect(paths).toContain("/main/README.md");
  expect(paths).toContain("/main/package.json");
  expect(paths).toContain("/main/src");

  // But NOT files inside subdirectories (that would be depth 3)
  expect(paths.some((p) => p.includes("/main/src/"))).toBe(false);
});

test("should list branches and nested contents at root with maxDepth: 3", async () => {
  const result = await afsGit.list("/", { maxDepth: 3 });
  ...
  // Should also have files inside src/ (depth 3)
  expect(paths).toContain("/main/src/index.ts");
  expect(paths).toContain("/main/src/utils.ts");
});

test("should respect limit at root with maxDepth", async () => {
  const result = await afsGit.list("/", { maxDepth: 3, limit: 5 });
