...
test("should list array of objects", async () => {
  ...
  // Check that array elements have children (objects)
  const firstUser = result.data.find((e) => e.path === "/users/0");
  expect(typeof firstUser?.meta?.childrenCount).toBe("number");
});
...
test("should read array object as node with children", async () => {
  const { data } = await afsJSON.read("/users/0");

  expect(typeof data?.meta?.childrenCount).toBe("number");
  expect(data?.meta?.childrenCount).toBe(3); // name, email, profile
...
test("should read nested object in array", async () => {
  ...
  expect(typeof data?.meta?.childrenCount).toBe("number");
  expect(data?.meta?.childrenCount).toBe(2); // age, city
});
...
test("should delete property from array object", async () => {
  ...
  // Verify it's gone
  await expect(afsJSON.read("/users/0/email")).rejects.toThrow("Path not found");
...
test("should handle mixed array content", async () => {
  ...
  // All items should be leaf nodes (no children)
  const items = result.data.filter((e) => e.path !== "/tags");
  items.forEach((item) => {
    expect(item.meta?.childrenCount).toBeUndefined();
  });
...
test("should rename property in array object", async () => {
  ...
  await expect(afsJSON.read("/users/0/name")).rejects.toThrow("Path not found");
...
test("should list all entries recursively", async () => {
  ...
  // Should include all nested paths (but NOT the root "/" - list() returns only children)
