...
test("AFSJSON should list root level entries", async () => {
  ...
  // Check childrenCount (defined for nodes with children, undefined for leaf nodes)
  const childrenInfo = result.data.map((entry) => ({
    path: entry.path,
    hasChildren: typeof entry.meta?.childrenCount === "number",
  }));
  expect(childrenInfo.sort((a, b) => a.path.localeCompare(b.path))).toMatchInlineSnapshot(`
    [
      {
        "hasChildren": true,
        "path": "/database",
...
test("AFSJSON should read an object as directory", async () => {
  ...
  expect(data?.meta?.childrenCount).toBe(3);
});
...
test("AFSJSON should throw for non-existent path", async () => {
  await expect(afsJSON.read("/nonexistent")).rejects.toThrow("Path not found");
});

test("AFSJSON should throw for invalid array index", async () => {
  await expect(afsJSON.read("/features/99")).rejects.toThrow("Path not found");
});
...
test("AFSJSON should write a deeply nested new value", async () => {
  const { data } = await afsJSON.write("/database/ssl/enabled", {
    content: true,
  });
  ...
  expect(typeof sslData?.meta?.childrenCount).toBe("number");
...
test("AFSJSON should delete a scalar value", async () => {
  ...
  // Verify it's gone
  await expect(afsJSON.read("/toDelete")).rejects.toThrow("Path not found");
});
...
test("AFSJSON should delete an array element", async () => {
  ...
  const { data } = await afsJSON.read("/features");
  expect(data?.meta?.childrenCount).toBe(2);

  // Index 2 should now be non-existent
  await expect(afsJSON.read("/features/2")).rejects.toThrow("Path not found");
});
...
test("AFSJSON should throw error when deleting directory without recursive", async () => {
  await expect(afsJSON.delete("/database")).rejects.toThrow(
...
test("AFSJSON should delete directory with recursive option", async () => {
  ...
  // Verify it's gone
  await expect(afsJSON.read("/deleteDir")).rejects.toThrow("Path not found");
});
...
test("AFSJSON should rename a value", async () => {
  ...
  await expect(afsJSON.read("/oldName")).rejects.toThrow("Path not found");
...
test("AFSJSON should throw error when renaming to existing path without overwrite", async () => {
  ...
  await expect(afsJSON.rename("/source", "/target")).rejects.toThrow(
...
test("AFSJSON should rename with overwrite option", async () => {
  ...
  const result = await afsJSON.rename("/source2", "/target2", {
    overwrite: true,
  });
  expect(result.message).toBe("Successfully renamed '/source2' to '/target2'");

  // Verify source is gone
  await expect(afsJSON.read("/source2")).rejects.toThrow("Path not found");
...
test("AFSJSON should search case-sensitively when specified", async () => {
  const result1 = await afsJSON.search("/", "LOCALHOST", {
    caseSensitive: true,
  });
  expect(result1.data.length).toBe(0);

  const result2 = await afsJSON.search("/", "localhost", {
    caseSensitive: true,
  });
...
test("AFSJSON should handle non-existent file", async () => {
  ...
  expect(typeof data?.meta?.childrenCount).toBe("number");
  expect(data?.meta?.childrenCount).toBe(0);
...
// Meta tests
test("AFSJSON should read root .meta", async () => {
  const { data } = await afsJSON.read("/.meta");

  expect(data).toBeDefined();
  expect(data?.path).toBe("/.meta");
  expect(data?.content).toMatchObject({
    type: "object",
    path: "/",
  });
  expect((data?.content as any).keys).toContain("database");
});

test("AFSJSON should read object .meta", async () => {
  const { data } = await afsJSON.read("/database/.meta");

  expect(data).toBeDefined();
  expect(data?.path).toBe("/database/.meta");
  expect(data?.content).toMatchObject({
    type: "object",
    path: "/database",
    childrenCount: expect.any(Number),
  });
  expect((data?.content as any).keys).toContain("host");
});

test("AFSJSON should read array .meta", async () => {
  const { data } = await afsJSON.read("/features/.meta");

  expect(data).toBeDefined();
  expect(data?.path).toBe("/features/.meta");
  expect(data?.content).toMatchObject({
    type: "array",
    path: "/features",
...
test("AFSJSON should read scalar .meta", async () => {
  const { data } = await afsJSON.read("/version/.meta");

  expect(data).toBeDefined();
  expect(data?.path).toBe("/version/.meta");
  expect((data?.content as any).type).toBe("string");
  expect((data?.content as any).value).toBeDefined();
});

test("AFSJSON should throw for non-existent .meta", async () => {
  await expect(afsJSON.read("/nonexistent/.meta")).rejects.toThrow();
});

// Meta write tests (metadata is written via node path, not .meta path)
test("AFSJSON should write and read meta for object", async () => {
  // Write meta via node path (new design: use payload.meta)
  await afsJSON.write("/database", { meta: { description: "Database config" } });

  // Read meta back
  const { data } = await afsJSON.read("/database/.meta");
  expect(data).toBeDefined();
  // User-defined metadata is in metadata field
  expect(data?.meta?.description).toBe("Database config");
  // Computed type info is in content field
  expect((data?.content as any).type).toBe("object");
});

test("AFSJSON should write and read meta for primitive", async () => {
  // First create a new primitive value
  await afsJSON.write("/testPrimitive", { content: "test value" });

  // Write meta via node path (new design)
  await afsJSON.write("/testPrimitive", { meta: { label: "Test Label" } });

  // Read meta back
  const { data } = await afsJSON.read("/testPrimitive/.meta");
  expect(data).toBeDefined();
  // User-defined metadata is in metadata field
  expect(data?.meta?.label).toBe("Test Label");
  // Computed type info is in content field
  expect((data?.content as any).type).toBe("string");
});

test("AFSJSON should not expose .afs in listings", async () => {
  // Write some meta to create .afs key
  await afsJSON.write("/database", { meta: { hidden: true } });

  // List should not include .afs
  const result = await afsJSON.list("/database");
  const paths = result.data.map((e) => e.path);
  expect(paths).not.toContain("/database/.afs");
});

test("AFSJSON should merge meta on write", async () => {
  // Write initial meta via node path
  await afsJSON.write("/metadata", { meta: { field1: "value1" } });

  // Write additional meta (should merge)
  await afsJSON.write("/metadata", { meta: { field2: "value2" } });

  // Read back - should have both fields in metadata
  const { data } = await afsJSON.read("/metadata/.meta");
  expect(data?.meta?.field1).toBe("value1");
  expect(data?.meta?.field2).toBe("value2");
});

test("AFSJSON should write content and metadata together", async () => {
  // Write both content (scalar) and metadata in one call
  await afsJSON.write("/combined", { content: "test-value", meta: { tag: "test" } });

  // Verify content (scalar value is returned in content field)
  const { data: readData } = await afsJSON.read("/combined");
  expect(readData?.content).toBe("test-value");

  // Verify metadata
  const { data: metaData } = await afsJSON.read("/combined/.meta");
  expect(metaData?.meta?.tag).toBe("test");
});
