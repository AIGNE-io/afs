...
describe("AFS-over-WebSocket proxy", () => {
  ...
  describe("cleanup on disconnect", () => {
    test("subscriptions are cleaned up when client disconnects", async () => {
      ...
      const writeResult = await afs!.write(`/ui/web/sessions/${sessionId}/pages/cleanup-test`, {
        content: "after disconnect",
        meta: { format: "html" },
      });

      // Write succeeds without throwing — proves subscription was cleaned up
      // (if subscription leaked, sending to closed socket would throw)
