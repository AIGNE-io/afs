...
describe("WebBackend caller identity", () => {
  ...
  describe("caller storage via injectConnection", () => {
    test("injectConnection with x-caller-did stores caller", () => {
      ...
      // Handshake to complete initialization
      conn.simulateMessage({ type: "join_session" });

      // Verify caller is stored (exposed via aupEventHandler)
      // We'll verify in the event handler tests below
      expect(true).toBe(true); // Connection accepted without error
    });

    test("injectConnection without caller headers stores no caller", () => {
      backend = new WebBackend({
        port: 0,
        inputSource: { readLine: () => new Promise(() => {}), hasPending: () => false } as any,
      });
    ...
    test("connection close cleans up caller entry", () => {
      backend = new WebBackend({
        port: 0,
        inputSource: { readLine: () => new Promise(() => {}), hasPending: () => false } as any,
      });
      ...
      conn.simulateMessage({ type: "join_session" });
      conn.simulateClose();
      // Caller entry should be cleaned up — verified by next test
