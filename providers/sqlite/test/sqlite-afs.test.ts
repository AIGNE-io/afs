...
describe("ActionsRegistry", () => {
  ...
  test("should list action names", () => {
    ...
    registry.registerSimple("action1", async () => ({ success: true }), {
      rowLevel: true,
    });
    registry.registerSimple("action2", async () => ({ success: true }), {
      tableLevel: true,
    });
  ...
  test("should execute actions", async () => {
    ...
    const ctx = {
      ...
      schemaService: {
        ...
        getPrimaryKeyColumn: () => "id",
      } as any,
      table: "test",
      pk: "1",
      module: { exportTable: async () => ({}) },
    };
  ...
  test("should list table level actions only", () => {
    ...
    registry.registerSimple("rowAction", async () => ({ success: true }), {
      rowLevel: true,
    });
    registry.registerSimple("tableAction", async () => ({ success: true }), {
      tableLevel: true,
    });
...
describe("SQLiteAFS Module", () => {
  ...
  beforeAll(async () => {
    ...
    const testDb = await afs.getDatabase();
    await testDb.run(
      sql.raw(`
  ...
  test("should list rows in table", async () => {
    ...
    expect(result.data.length).toBeGreaterThan(0);
    // First entry is the table itself (self), find first row entry
    const rowEntry = result.data.find((e) => e.content !== undefined);
    expect(rowEntry?.content).toHaveProperty("name");
  });
  ...
  test("should throw AFSNotFoundError for invalid list path", async () => {
    await expect(afs.list("/nonexistent")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("should throw AFSNotFoundError for invalid read path", async () => {
    await expect(afs.read("/nonexistent/path")).rejects.toBeInstanceOf(AFSNotFoundError);
  });
  ...
  test("should get schemas map", async () => {
  ...
  test("should get database instance", async () => {
    const database = await afs.getDatabase();
...
describe("SQLiteAFS Static Methods", () => {
  ...
  test("should load from config", async () => {
    const afs = await SQLiteAFS.load({
      basePath: "/test/path",
      config: { url: ":memory:", name: "test-db" },
    });
...
describe("SQLiteAFS Readonly Mode", () => {
  ...
  beforeAll(async () => {
    ...
    await readonlyAfs.ensureInitialized();

    const testDb = await readonlyAfs.getDatabase();
    await testDb.run(
      sql.raw(`
...
describe("SQLiteAFS Search", () => {
  ...
  beforeAll(async () => {
    ...
    await searchAfs.ensureInitialized();

    const testDb = await searchAfs.getDatabase();
    await testDb.run(
      sql.raw(`
...
describe("SQLiteAFS Exec", () => {
  ...
  beforeAll(async () => {
    ...
    await execAfs.ensureInitialized();

    const testDb = await execAfs.getDatabase();
    await testDb.run(
      sql.raw(`
  ...
  test("should throw on exec for unsupported path", async () => {
    await expect(execAfs.exec("/", {}, {})).rejects.toThrow("No exec handler for path: /");
  });
...
describe("SQLiteAFS Custom Actions", () => {
  ...
  beforeAll(async () => {
    ...
    await actionAfs.ensureInitialized();

    const testDb = await actionAfs.getDatabase();
    await testDb.run(
      sql.raw(`
...
describe("SQLiteAFS Export Table", () => {
  ...
  beforeAll(async () => {
    ...
    await exportAfs.ensureInitialized();

    const testDb = await exportAfs.getDatabase();
    await testDb.run(
      sql.raw(`
  ...
  test("should throw AFSNotFoundError for non-existent table (JSON export)", async () => {
    await expect((exportAfs as any).exportTable("nonexistent", "json")).rejects.toBeInstanceOf(
      AFSNotFoundError,
    );
  });

  test("should throw AFSNotFoundError for non-existent table (CSV export)", async () => {
    await expect((exportAfs as any).exportTable("nonexistent", "csv")).rejects.toBeInstanceOf(
      AFSNotFoundError,
    );
  });
...
describe("Query Builder", () => {
  ...
  test("buildUpdate should exclude PK column", () => {
    const query = buildUpdate("test", mockSchema, "1", {
      id: 999,
      name: "Updated",
    });
...
describe("CRUDOperations", () => {
  let crudDb: SqliteDatabase;
  let crud: CRUDOperations;

  beforeAll(async () => {
    crudDb = await initDatabase({
      url: ":memory:",
    });
    await crudDb.run(
      sql.raw(`
    ...
    // Use SchemaService instead of cached schemas
    const { SchemaService } = await import("../src/schema/service.js");
    const schemaService = new SchemaService(crudDb);
    crud = new CRUDOperations(crudDb, schemaService, "");
  });

  test("hasTable returns true for existing table", async () => {
    expect(await crud.hasTable("items")).toBe(true);
  });

  test("hasTable returns false for non-existing table", async () => {
    expect(await crud.hasTable("nonexistent")).toBe(false);
  });

  test("getTableSchema returns schema for existing table", async () => {
    const schema = await crud.getTableSchema("items");
    expect(schema).toBeDefined();
    expect(schema?.name).toBe("items");
  });

  test("getTableSchema returns undefined for non-existing table", async () => {
    const schema = await crud.getTableSchema("nonexistent");
    expect(schema).toBeUndefined();
  });

  test("listTable throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.listTable("nonexistent")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("readRow throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.readRow("nonexistent", "1")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("readRow throws AFSNotFoundError for non-existing row", async () => {
    await expect(crud.readRow("items", "9999")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("getMeta throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.getMeta("nonexistent", "1")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("getMeta throws AFSNotFoundError for non-existing row", async () => {
    await expect(crud.getMeta("items", "9999")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("createRow throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.createRow("nonexistent", { name: "Test" })).rejects.toBeInstanceOf(
      AFSNotFoundError,
    );
  });

  test("updateRow throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.updateRow("nonexistent", "1", { name: "Test" })).rejects.toBeInstanceOf(
      AFSNotFoundError,
    );
  });

  test("deleteRow throws AFSNotFoundError for non-existing table", async () => {
    await expect(crud.deleteRow("nonexistent", "1")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("deleteRow throws AFSNotFoundError for non-existing row", async () => {
    await expect(crud.deleteRow("items", "9999")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  // Note: setSchemas was removed - SchemaService queries on-demand
});

describe("FTSSearch", () => {
  let ftsDb: SqliteDatabase;
  let schemaService: any;
  let _ftsSearch: FTSSearch;

  beforeAll(async () => {
    ftsDb = await initDatabase({
      url: ":memory:",
    });
    await ftsDb.run(
      sql.raw(`
...
describe("Node Builder", () => {
  ...
  test("buildRowEntry creates correct entry", () => {
    ...
    expect(entry.meta?.table).toBe("test");
  ...
  test("buildTableEntry creates correct entry", () => {
    ...
    expect(entry.meta?.columnCount).toBe(3);
    expect(entry.meta?.childrenCount).toBe(100);
  });
  ...
  test("buildActionsListEntry creates entries for all actions", () => {
    const actions = [
      { name: "validate", description: "Validate row" },
      { name: "duplicate", description: "Duplicate row" },
      { name: "custom" },
    ];
    const entries = buildActionsListEntry("test", "1", actions);
...
describe("Config Schema", () => {
  ...
  test("accepts config without url (defaults to :memory:)", () => {
    const result = sqliteAFSConfigSchema.safeParse({});
    expect(result.success).toBe(true);
  });
...
describe("Built-in Actions Integration", () => {
  ...
  beforeAll(async () => {
    ...
    actionDb = await afs.getDatabase();
    await actionDb.run(
      sql.raw(`
  ...
  test("validate action passes for valid row", async () => {
    const result = await afs.write("/products/1/.actions/validate", {
      content: {},
    });
  ...
  test("duplicate action creates new row", async () => {
    const result = await afs.write("/products/1/.actions/duplicate", {
      content: {},
    });
    expect((result.data.content as Record<string, unknown>).success).toBe(true);
  ...
  test("validate action with foreign key check", async () => {
    ...
    const phone = listResult.data.find((p) => p.content && (p.content as any).name === "Phone");

    if (phone) {
      const pkValue = (phone.content as any).id;
      const result = await afs.write(`/products/${pkValue}/.actions/validate`, {
        content: {},
      });
      expect((result.data.content as Record<string, unknown>).success).toBe(true);
      expect((result.data.content as Record<string, unknown>).data).toHaveProperty("valid", true);
...
describe("Built-in Actions - Row Level Actions", () => {
  ...
  test("validate action on specific row", async () => {
    const result = await afs.write("/orders/1/.actions/validate", {
      content: {},
    });
    expect((result.data.content as Record<string, unknown>).success).toBe(true);
    expect((result.data.content as Record<string, unknown>).data).toHaveProperty("valid", true);
  ...
  test("duplicate action creates a copy of the row", async () => {
    const result = await afs.write("/orders/1/.actions/duplicate", {
      content: {},
    });
    expect((result.data.content as Record<string, unknown>).success).toBe(true);
...
describe("Validate Action Edge Cases", () => {
  ...
  test("validate detects invalid foreign key reference", async () => {
    ...
    const orphan = listResult.data.find((i) => i.content && (i.content as any).name === "Orphan");
...
describe("Built-in Actions - Direct Registry Tests", () => {
  ...
  test("duplicate action throws AFSNotFoundError for non-existent row", async () => {
    // Non-existent row throws AFSNotFoundError
    await expect(
      afs.write("/registry_test/9999/.actions/duplicate", { content: {} }),
    ).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("validate action throws AFSNotFoundError for non-existent row", async () => {
    // Non-existent row throws AFSNotFoundError
    await expect(
  ...
  test("validate action succeeds for valid row", async () => {
    ...
    const result = await afs.write("/registry_test/1/.actions/validate", {
      content: {},
    });
    const content = result.data.content as Record<string, unknown>;
    expect(content.success).toBe(true);
    expect((content.data as Record<string, unknown>).valid).toBe(true);
    expect((content.data as Record<string, unknown>).errors).toEqual([]);
  });
});
...
describe("formatValueForSQL Coverage", () => {
  ...
  test("duplicate handles boolean values", async () => {
    ...
    const row = listResult.data.find(
      (r) => r.content && (r.content as any).name === "Boolean Test",
    );
  ...
  test("duplicate handles Date values", async () => {
    ...
    const listResult = await afs.list("/format_test");
    const row = listResult.data.find((r) => r.content && (r.content as any).name === "Date Test");
  ...
  test("duplicate handles object/JSON values", async () => {
    ...
    const listResult = await afs.list("/format_test");
    const row = listResult.data.find((r) => r.content && (r.content as any).name === "JSON Test");
  ...
  test("duplicate handles null values", async () => {
    ...
    const listResult = await afs.list("/format_test");
    const row = listResult.data.find((r) => r.content && (r.content as any).name === "Null Test");
...
describe("SQLiteAFS Stat", () => {
  let statAfs: SQLiteAFS;

  beforeAll(async () => {
    statAfs = new SQLiteAFS({ url: ":memory:" });
    await statAfs.ensureInitialized();

    const testDb = await statAfs.getDatabase();
    await testDb.run(
      sql.raw(`
    ...
    await testDb.run(sql.raw(`INSERT INTO stat_users (name) VALUES ('Alice')`));
    await testDb.run(sql.raw(`INSERT INTO stat_users (name) VALUES ('Bob')`));
  });

  test("stat() on root returns database info with actions", async () => {
    const result = await statAfs.stat("/");
    expect(result.data).toBeDefined();
    expect(result.data?.path).toBe("/");
    expect(result.data?.meta?.childrenCount).toBe(1); // stat_users table
    expect(result.data?.meta?.kind).toBe("sqlite:database");
    expect(result.data?.meta?.kinds).toContain("sqlite:database");
    expect(result.data?.meta?.kinds).toContain("afs:node");
    // Root-level actions may or may not exist depending on registration
    // Just check the structure is correct if actions exist
  ...
  test("stat() on table returns table info with actions", async () => {
    const result = await statAfs.stat("/stat_users");
    expect(result.data).toBeDefined();
    expect(result.data?.path).toBe("/stat_users");
    expect(result.data?.meta?.childrenCount).toBe(2); // 2 rows
    expect(result.data?.meta?.kind).toBe("sqlite:table");
    expect(result.data?.meta?.kinds).toContain("sqlite:table");
    expect(result.data?.meta?.table).toBe("stat_users");
    expect(result.data?.meta?.columnCount).toBe(2);
    // Table-level actions
    expect(result.data?.actions).toBeDefined();
    expect(Array.isArray(result.data?.actions)).toBe(true);
    expect(result.data?.actions?.length).toBeGreaterThan(0);
    // Check export action exists
    const exportAction = result.data?.actions?.find((a) => a.name === "export");
    expect(exportAction).toBeDefined();
    expect(exportAction?.description).toBeDefined();
  });

  test("stat() on row returns row info with actions", async () => {
    const result = await statAfs.stat("/stat_users/1");
    expect(result.data).toBeDefined();
    expect(result.data?.path).toBe("/stat_users/1");
    expect(result.data?.meta?.childrenCount).toBe(0); // rows have no children
    expect(result.data?.meta?.kind).toBe("sqlite:row");
    expect(result.data?.meta?.kinds).toContain("sqlite:row");
    expect(result.data?.meta?.table).toBe("stat_users");
    expect(result.data?.meta?.primaryKey).toBe("1");
    // Row-level actions
    expect(result.data?.actions).toBeDefined();
    expect(Array.isArray(result.data?.actions)).toBe(true);
    expect(result.data?.actions?.length).toBeGreaterThan(0);
    // Check duplicate action exists
    const duplicateAction = result.data?.actions?.find((a) => a.name === "duplicate");
    expect(duplicateAction).toBeDefined();
  });

  test("stat() on non-existent table throws AFSNotFoundError", async () => {
    await expect(statAfs.stat("/nonexistent")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("stat() on non-existent row throws AFSNotFoundError", async () => {
    await expect(statAfs.stat("/stat_users/999")).rejects.toBeInstanceOf(AFSNotFoundError);
  });

  test("stat() actions field is undefined when no actions available", async () => {
    // For paths that don't support actions (if any), actions should be undefined
    // In SQLite provider, all paths support actions, so this test validates the structure
    const result = await statAfs.stat("/stat_users");
  ...
  test("stat() action summaries have name and optional description", async () => {
    const result = await statAfs.stat("/stat_users");
    expect(result.data?.actions).toBeDefined();
    for (const action of result.data?.actions ?? []) {
