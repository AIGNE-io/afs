...
describe("performIssuerAdd", () => {
  ...
  test("no source flag → throws RUNTIME_ERROR", async () => {
    expect(performIssuerAdd({ name: "no-source", home: tempHome })).rejects.toThrow(
      "No input source",
    );
  });

  test("--did without --pk → throws", async () => {
    expect(performIssuerAdd({ name: "half", home: tempHome, did: "z1Test" })).rejects.toThrow(
      "Both --did and --pk required",
    );
  });

  test("--pk without --did → throws", async () => {
    expect(performIssuerAdd({ name: "half", home: tempHome, pk: "0x04test" })).rejects.toThrow(
      "Both --did and --pk required",
    );
  });
  ...
  test("--from-key missing did field → throws", async () => {
    ...
    expect(performIssuerAdd({ name: "no-did", home: tempHome, fromKey: keyPath })).rejects.toThrow(
      "Key file missing 'did' field",
    );
  });
  ...
  test("--from-key missing pk field → throws", async () => {
    ...
    expect(performIssuerAdd({ name: "no-pk", home: tempHome, fromKey: keyPath })).rejects.toThrow(
      "Key file missing 'pk' field",
    );
  });
...
describe("performIssuerAdd --from-vc", () => {
  ...
  test("VC with no proof → throws", async () => {
    ...
    expect(performIssuerAdd({ name: "vc-np", home: tempHome, fromVc: vcPath })).rejects.toThrow(
      "No proof array in VC",
    );
  });
  ...
  test("VC with only self-sign proof → throws", async () => {
    ...
    expect(performIssuerAdd({ name: "vc-so", home: tempHome, fromVc: vcPath })).rejects.toThrow(
      "No counter-sign proof found in VC",
    );
  });
  ...
  test("proof missing signer field → throws", async () => {
    ...
    expect(performIssuerAdd({ name: "vc-ns", home: tempHome, fromVc: vcPath })).rejects.toThrow(
      "No counter-sign proof found",
    );
  });
