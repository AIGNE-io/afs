 ...
 * Simulates /blocklet/seed/ (read-only source) and /data/ (writable target).
 */
function createMockAFS(
  ...
  const afs = {
    async list(path: string) {
      ...
      if (normalized.startsWith("/blocklet/seed/")) {
        return { data: listDir(seedFiles, normalized.replace("/blocklet/seed/", "/")) };
      }
      if (normalized === "/blocklet/seed/") {
    ...
    async read(path: string) {
      const seedMatch = path.replace(/^\/blocklet\/seed/, "");
...
describe("seedBlockletData", () => {
  test("seeds files from /blocklet/seed/ to /data/", async () => {
