...
function createMockAFS(
  ...
  const afs = {
    async list(path: string) {
      ...
      if (normalized === "/program/.route") {
        return {
          data: Object.keys(programRoutes).map((name) => ({
            path: `/program/.route/${name}`,
            type: "file" as const,
            meta: {},
          })),
        };
    ...
    async read(path: string) {
      const programMatch = path.match(/^\/program\/\.route\/(.+)$/);
      if (programMatch) {
...
describe("seedRoutes (T2-1)", () => {
  test("seeds routes from /program/.route/ when /data/.route/ is empty", async () => {
  ...
  test("does NOT seed when /program/.route/ does not exist", async () => {
    const { afs } = createMockAFS();
