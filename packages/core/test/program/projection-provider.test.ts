...
describe("createBlockletAFS", () => {
  ...
  describe("Happy Path", () => {
    ...
    test("/program path is readable (readonly)", async () => {
      const hostAFS = await setupHostAFS({
        mountUris: { ash: "ash://", aignehub: "aignehub://" },
      });
      ...
      const result = await afs.read!("/program/scripts/main.ash");
      expect(result).toBeDefined();
    });

    test("/program path rejects write (readonly enforced)", async () => {
      const hostAFS = await setupHostAFS({
        mountUris: { ash: "ash://", aignehub: "aignehub://" },
      });
      ...
      await expect(afs.write!("/program/file.txt", { content: "hack" })).rejects.toThrow();
    });
...
`;
  ...
  // =========================================================================
  describe("Edge Cases", () => {
    test("manifest with no mounts only creates /program and /data", async () => {
      const noMountsYaml = `
specVersion: 1
id: minimal
name: Minimal
...
`;
      ...
      // /program and /data should work
      const readResult = await afs.read!("/program/main.ash");
      expect(readResult).toBeDefined();
    });
...
`;
  ...
  // =========================================================================
  describe("Security", () => {
    test("/program is strictly readonly — cannot write via any path", async () => {
      const hostAFS = await setupHostAFS({
        mountUris: { ash: "ash://", aignehub: "aignehub://" },
      });
      ...
      await expect(afs.write!("/program/hack.txt", { content: "pwned" })).rejects.toThrow();
      await expect(afs.delete!("/program/scripts/main.ash")).rejects.toThrow();
    });
  });
