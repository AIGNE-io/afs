...
describe("createBlockletAFS — /data direct mount via createDataProvider", () => {
  describe("Happy Path", () => {
    ...
    it("/blocklet still uses ProjectionProvider (readonly)", async () => {
      const dataProvider = createMockProvider({ name: "data-fs", files: {} });
      const yaml = buildProgramYaml();
      const { globalAFS, programPath } = await setupGlobalAFS({ programYaml: yaml });
      ...
      const runtimeAFS = result.afs as AFS;
      // Read program.yaml through /blocklet path
      const readResult = await runtimeAFS.read!("/blocklet/program.yaml");
  ...
  describe("Bad Path", () => {
    ...
    it("/blocklet rejects write operations (AFSAccessModeError)", async () => {
      const dataProvider = createMockProvider({ name: "data-fs" });
      const yaml = buildProgramYaml();
      const { globalAFS, programPath } = await setupGlobalAFS({ programYaml: yaml });
      ...
      const runtimeAFS = result.afs as AFS;
      await expect(runtimeAFS.write!("/blocklet/test.txt", { content: "hack" })).rejects.toThrow();
    });
  });
