...
describe("Trust Gate", () => {
  ...
  describe("no trust config → no checking", () => {
    test("mount without trust config does not call checkTrustGate", () => {
      // This is tested by the fact that AFS only calls checkTrustGate when
      // this.trustConfig is set. Verified by the integration test below.
