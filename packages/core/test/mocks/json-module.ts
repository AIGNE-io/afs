...
export interface AFSJSONOptions {
  ...
  /**
   * Visibility level for this module.
   * - "full": No restrictions (default)
   * - "meta": read() strips content, search() denied
   * @default "full"
   */
  visibility?: AFSVisibility;
  /**
  ...
  async list(path: string, options?: AFSListOptions): Promise<AFSListResult> {
    ...
    while (queue.length > 0) {
      ...
      if (entries.length >= limit) {
        if (entry.meta) {
          entry.meta.childrenTruncated = true;
        }
        break;
      }
