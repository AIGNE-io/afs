...
export class AFS implements AFSRoot {
  ...
  private async execActivatedProgram(
    ...
    // Read entrypoint source from /program mount (runtime AFS convention).
    // The ProjectionProvider at /program uses sourceModule (the original provider)
    ...
    const entrypointPath = joinURL("/program", manifest.entrypoint);
