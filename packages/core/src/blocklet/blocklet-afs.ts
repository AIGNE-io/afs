 ...
 *
 * Mounts:
 * - `/blocklet` → blockletPath (readonly via allowedOps)
...
export async function createBlockletAFS(
  ...
  // 3. Mount /blocklet → blockletPath (readonly)
  ...
  await blockletAFS.mount(blockletProvider, "/blocklet", { lenient: true });
  ...
  if (options?.mountOverrides && options.createProvider) {
    ...
    const reservedPaths = new Set(["/blocklet", "/data"]);
